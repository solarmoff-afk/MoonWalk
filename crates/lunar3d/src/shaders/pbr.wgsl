...
fn calculate_shadow(light_index: u32, world_pos: vec3<f32>, N: vec3<f32>, L: vec3<f32>) -> f32 {
    let bias_vec = N * 0.02; 
    let shadow_pos = global.light_view_projs[light_index] * vec4<f32>(world_pos + bias_vec, 1.0);
    ...
    let bias = max(0.0001 * (1.0 - dot(N, L)), 0.00001);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    for (var i = 0u; i < global.num_lights; i = i + 1u) {
        ...
        let attenuation = 1.0 / (1.0 + 0.045 * dist + 0.0075 * dist * dist);
    ...
    let ambient_strength = 0.5 + 0.5 * N.y;
    let ambient = global.ambient_color * albedo * ambient_strength; 
