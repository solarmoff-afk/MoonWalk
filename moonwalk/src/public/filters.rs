...
impl MoonWalk {
    ...
    pub fn blur_texture(&mut self, texture_id: TextureId, radius: f32, horizontal: bool) {
        self.renderer.apply_blur(texture_id.0, radius, horizontal);
    }
    ...
    pub fn chromakey(&mut self, texture_id: TextureId, key_color: Vec3, tolerance: f32) {
        self.renderer.apply_chromakey(texture_id.0, key_color.to_array(), tolerance);
    }
    ...
    pub fn apply_mask(&mut self, target_id: TextureId, mask_id: u32, invert: bool) {
        self.renderer.apply_stencil(target_id.0, mask_id, invert);
    }
    ...
    pub fn color_matrix(&mut self, texture_id: TextureId, matrix: Mat4, offset: Vec4) {
        let mat_arr = matrix.to_cols_array_2d();
        let off_arr = offset.to_array();

        self.renderer.apply_color_matrix(texture_id.0, mat_arr, off_arr);
    }
    ...
    pub fn mesh_gradient(
        ...
        self.renderer.apply_mesh_gradient(
            texture_id.0, colors, pos, noise_intensity, warp_strength,
            warp_phase, gamma, normal_blend_mode
        );
    }

    /// 
    pub fn sdf_mask(&mut self, texture_id: TextureId, radius: f32, hardness: f32) {
        self.renderer.apply_sdf_mask(texture_id.0, radius, hardness);
    }
}
